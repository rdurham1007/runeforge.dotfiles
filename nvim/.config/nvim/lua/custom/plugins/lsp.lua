    ...
    vim.api.nvim_create_autocmd('LspAttach', {
      ...
      callback = function(event)
        ...
        vim.keymap.set('n', '<leader>bp', function()
          vim.notify('Building project...', vim.log.levels.INFO, { title = 'Build' })
          -- Run makeprg (dotnet build)
          ...
          if #qfl > 0 then
            vim.cmd 'copen'
            vim.notify('Build finished with errors', vim.log.levels.WARN, { title = 'Build' })
          else
            vim.notify('Build succeeded', vim.log.levels.INFO, { title = 'Build' })
          end
        end, { desc = 'Build project (dotnet)' })
    ...
    local function copilot_sign_out(_, client)
      client:request('signOut', vim.empty_dict(), function(err, result)
        ...
        if result.status == 'NotSignedIn' then
          ...
          vim.notify 'Signed out successfully.'
        end
      end)
    end
